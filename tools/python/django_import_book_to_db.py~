...
for style in styles:
	print "%s\t%s" % (style.id,style.name)

style_selecteds=None

err=''
while True:
	print "%s请选择此书的分类ID: " % err,
	style_id=int(raw_input())

	style_selecteds = [i for i in styles if i.id==style_id]

	err='发现错误，'
	if len(style_selecteds):
		break

book = helper.getNewBook(name=bookName,catalog=catalog_selecteds[0],style=style_selecteds[0],author=author,tag=keyword)
